...
function FlashcardRoom() {
  ...
  useEffect(() => {
    const fetchFlashcards = async () => {
      ...
      try {
        const response = await fetch("https://summa-pdf-backend.onrender.com/flashcards", {
          method: "POST",
  ...
  if (loading) {
    content = <p>Loading flashcards...</p>;
  ...
  >
    <div className="max-h-full overflow-auto">{flashcards[currentIndex].term}</div>
  </div>
  ...
  <div
    ...
    onClick={() => setIsFlipped(!isFlipped)}
  >
    <div className="max-h-full overflow-auto">{flashcards[currentIndex].definition}</div>
  </div>
</ReactCardFlip>
        ...
        <div className="flex gap-4 mt-6">
          <button title="prev"
            onClick={prevCard}
            className="bg-gray-700 text-white px-8 py-4 rounded-md shadow-md hover:bg-gray-900 transition"
          >
            <FaArrowLeftLong/>
          </button>
          <button title="next"
            onClick={nextCard}
            className="bg-primary text-white px-8 py-4 rounded-md shadow-md hover:bg-primary-dark transition"
          >
            <FaArrowRightLong/>
          </button>
        </div>
  ...
  return (
    <div className=" bg-gray-100">
      <Header />
      <h1 className="text-2xl font-[700] text-primary m-6">Flashcards Room</h1>
